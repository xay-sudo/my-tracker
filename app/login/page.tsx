...
export default function LoginPage() {
  ...
  const [isSignUp, setIsSignUp] = useState(false); // Switch between Login & Signup
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState('');

  // 1. Handle Email + Password Logic
  const handleAuth = async (e: any) => {
    ...
    if (isSignUp) {
      ...
      if (error) {
        ...
        // If "Confirm Email" is OFF in Supabase, this logs them in immediately
  ...
  // 2. Handle Google Login (Kept from before)
